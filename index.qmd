---
title: "Homework 2"
author: "[IAdvait Ashtikar]{style='background-color: yellow;'}"
toc: true
...
```{r}
plt <- ggplot(df, aes(x = length)) + geom_histogram(bins = 30)
plt
```
...
```{r}
plt4 <- ggplot(df, aes(x = length, y = diameter, color = sex)) + geom_point()
plt4
```
...
```{r}
plt5 <- ggplot(df, aes(x = length, y = diameter)) + geom_point(aes(color = sex)) + geom_smooth(method = "lm", se = FALSE) + facet_wrap(~sex, ncol = 1)
plt5
```
...
quant_vars <- names(df) %>%
  ...
  set_names(paste0("Quantitative Variable: ",.))

scatter_plots <- quant_vars %>%
  map2(quant_vars, ~{
    ggplot(df, aes_string(x = .x, y = "rings", color = "sex")) + geom_point() + labs(x = .y, y = "Number of Rings")
  })

cowplot::plot_grid(plotlist = scatter_plots, ncol = 2)
```
